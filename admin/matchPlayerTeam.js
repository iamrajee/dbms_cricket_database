...
const specialRoles = ['None', 'wicket-keeper', 'captain', 'extra']
...
function getBlock(req, res){
    const table_name = "MatchPlayerTeam"
    observerColumns = tableInfo.getInfosObservable(con, table_name)
    observerColumns.subscribe(
        v => { 
            console.log("Query Successful")
            res.render("admin/matchPlayerTeam/matchPlayerTeamInsert", {specialRoles: specialRoles},
            (err, html)=>{
            if(err) throw err
...
function insertIntoTable(req, res){
    ...
    const special_role = req.body.special_role

    console.log("Information to be Inserted: ")
    console.log("batting scorecard")
    const sql_query = "INSERT INTO MatchPlayerTeam("
                    + "match_id, player_id, special_role"
                    + "Values(?, ?, ?, ?, ?, ?)"
    console.log(sql_query) 
    con.query(sql_query, [match_id, player_id, special_role], 
        (err, results, fields)=>{
            if(err){
                res.send("Error")
                throw err
