...
function getBlock(req, res){
    ...
    observerColumns.subscribe(
        v => { 
            ...
            res.render("admin/teams/teamsInsert",
            (err, html)=>{
                if(err) throw err
...
function insertIntoTable(req, res){
    ...
    console.log(team_name, authority)
    const sql_query = "INSERT INTO Teams(team_name, authority)" 
                        + "Values(?, ?)"
    console.log(sql_query)
    con.query(sql_query, [team_name, authority], (err, results, fields)=>{
        if(err){
            res.send("Error")
            throw err
        } 
